...
- Hardened keybind plugin pipe delivery semantics for multi-client sessions:
  - In `pipe_to_specific_plugins`, for keybind-origin messages (`PipeSource::Keybind`) with known source client, route to that client's plugin instance when available instead of fanning out to all instances for the same plugin location.
  - Rationale: reduce duplicate per-keybind/plugin work across client-scoped plugin instances and avoid diverging behavior across copies while preserving CLI broadcast-style behavior.
- `W3`-`W7` validation:
